...
def _read_y_from_yuv_worker(video_path, yuv_type, h, w, index_frame, key, compress_level, date_type):
    """不要把该函数放到主函数里，否则无法并行。"""
    if h == None:
...
def make_y_lmdb_from_yuv(
        ...
        batch=7000, compress_level=1, multiprocessing_read=False, map_size=None, date_type=np.uint8
):
    # check
    assert lmdb_path.endswith('.lmdb'), "lmdb_path must end with '.lmdb'."
