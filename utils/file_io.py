...
def import_yuv(seq_path, h, w, tot_frm, yuv_type='420p', start_frm=0, only_y=False, only_u = False, only_v=False,date_type=np.uint8):
    ...
    with open(seq_path, 'rb') as fp:
        ...
        for i in range(tot_frm):
            ...
            u_frm = np.fromfile(fp, dtype=date_type, count=u_size).reshape(hh, ww)
            v_frm = np.fromfile(fp, dtype=date_type, count=v_size).reshape(hh, ww)
