...
# path of ground truth videos (yuv8bits)
gt_dir = '/home/shuchen/datasets/ScreenContentDataset/RAW/test/8bit'

# paths of VTM12.1 LDP compressed low quality videos with 4 QPs (yuv10bits)
...
def test(raw_yuv_path,lq_yuv_path,sd_path,qp):
    model = Net(frame_channels = 1, mid_channels=64)  # Create a STA network
    ...
    nfs = int(nfs)
    w, h = int(wxh.split('x')[0]), int(wxh.split('x')[1])  # Parameters of video
    ...
    lq_y = lq_y.astype(np.float32) / 4 / 255.
    print("            yuv load complete")
    ...
    # Start enhance
    for index in range(0, nfs):  # for each frame
        torch.cuda.empty_cache()
        # Take 5 adjacent frames as input, and if it exceeds the boundary, repeat the outermost frame to make up for 5 frames
        ...
        # Enhance 1 frame with the help of 5 frames
        with torch.no_grad():
...
def main():
    for i in [0,1,2,3]:  # 0:QP22 1:QP27 2:QP32 3:QP37
        ...
        for xxxx in range(len(gt_video_list)):
            raw_yuv_path = gt_video_list[xxxx]
